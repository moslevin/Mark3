...
#define DCPU_NORMAL_OPCODE_MASK(x) ((uint16_t)(x & 0x001F))

#define DCPU_EXTENDED_OPCODE_MASK(x) ((uint16_t)((x >> 5) & 0x001F))

#define DCPU_A_MASK(x) ((uint16_t)((x >> 10) & 0x003F))

#define DCPU_B_MASK(x) ((uint16_t)((x >> 5) & 0x001F))
...
typedef enum {
    ...
    OP_DIV, //!< b, a | sets b to b/a, sets EX to ((b<<16)/a)&0xffff. if a==0, sets b and EX to 0 instead. (treats b, a
            //! as unsigned)
...
typedef enum {
    ...
    OP_EX_IAQ, //!< if a is nonzero, interrupts will be added to the queue instead of triggered. if a is zero,
...
public:
     ...
     */
    void Init(
        uint16_t u16DeviceNumber_, uint32_t u32HWID_, uint32_t u32VID_, uint16_t u16Version_, DCPU_Callback pfCallback_)
    {
        m_u32HWID         = u32HWID_;
        m_u32VID          = u32VID_;
     ...
     */
    void Interrupt(DCPU* pclCPU_) { m_pfCallback(pclCPU_); }
    /*!
     ...
     */
    uint16_t GetDeviceNumber() { return m_u16DeviceNumber; }
