...
//---------------------------------------------------------------------------
bool TerminalInput::ParseCharacter(char cInput_)
{
    return RunHandler(cInput_);
}

//---------------------------------------------------------------------------
void TerminalInput::GetLastKey(KeyVal_t* pstKeyVal_)
{
    *pstKeyVal_ = m_uKey;
}

//---------------------------------------------------------------------------
bool TerminalInput::RunHandler(char cInput_)
{
    switch (m_eState) {
    case terminal_parser_begin:                     return BeginHandler(cInput_);
    case terminal_parser_escape_check:              return EscapeCheckHandler(cInput_);
    case terminal_parser_escape_type:               return EscapeTypeHandler(cInput_);
    case terminal_parser_escape:                    return EscapeHandler(cInput_);
    case terminal_parser_escape_modified:           return EscapeModifierHandler(cInput_);
    case terminal_parser_escape_expect_semi:        return ExpectSemiHandler(cInput_);
    }
    return false;
}

//---------------------------------------------------------------------------
void TerminalInput::AcceptSpecial(terminal_key_t eKey_)
{
    m_uKey.eKey = eKey_;
...
//---------------------------------------------------------------------------
void TerminalInput::AcceptCharacter(char cChar_)
{
    m_uKey.cChar = cChar_;
    m_uKey.eModifier = (terminal_key_modifier_t)(m_u8Modifier);
...
//---------------------------------------------------------------------------
bool TerminalInput::BeginHandler(char cInput_)
{
    m_u8Modifier = 0;
    m_u8Command = 0;
    m_u8EscapeIdx = 0;
...
//---------------------------------------------------------------------------
bool TerminalInput::EscapeCheckHandler(char cInput_)
{
...
//---------------------------------------------------------------------------
bool TerminalInput::EscapeTypeHandler(char cInput_)
{
    if (m_cEscapeBegin == '^') {
        switch (cInput_) {
        case 'C': {
...
//---------------------------------------------------------------------------
bool TerminalInput::EscapeHandler(char cInput_)
{
    switch (cInput_) {
    case ';': {
        uint8_t cmd = 0;
...
//---------------------------------------------------------------------------
bool TerminalInput::EscapeModifierHandler(char cInput_)
{
    if (cInput_ == ';') {
...
//---------------------------------------------------------------------------
bool TerminalInput::ExpectSemiHandler(char cInput_)
{
    switch (cInput_) {
        case '0':
        case '1':
